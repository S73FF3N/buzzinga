...
def button(text, x, y, w, h, click, inactive_color=Static.RED, active_color=Static.LIGHT_RED, text_color=Static.WHITE,
           image=False):
...
def player_name_input(x, y, w, h, click, inactive_color=Static.RED, active_color=Static.LIGHT_RED,
                      text_color=Static.RED):
    mouse = pygame.mouse.get_pos()
    return_value = False
    if x < mouse[0] < x + w and y < mouse[1] < y + h:
...
def toggle_btn(text, text2, x, y, w, h, click, text_color=Static.RED, enabled=True, draw_toggle=True, blit_text=True,
               enabled_color=Static.LIGHT_RED):
    mouse = pygame.mouse.get_pos()
    rect_height = h // 2
    text_surf, text_rect = text_objects(text, SMALL_TEXT, color=text_color)
    if rect_height % 2 == 0:
    ...
    if enabled and draw_toggle:
        pygame.draw.rect(SCREEN, enabled_color,
                         (x + text_rect.width + int(SCREEN_WIDTH / 50), y + int(h / 4), TOGGLE_ADJ, rect_height))
        draw_circle(SCREEN, int(x + text_rect.width + SCREEN_WIDTH / 50), y + int(h / 2), int(h // 4), enabled_color)
        draw_circle(SCREEN, int(x + text_rect.width + SCREEN_WIDTH / 50 + h / 4 + TOGGLE_ADJ / 2), y + int(h / 2),
                    int(h // 4), enabled_color)
        draw_circle(SCREEN, int(x + text_rect.width + SCREEN_WIDTH / 50 + h / 4 + TOGGLE_ADJ / 2), y + int(h / 2),
                    int(h // 5), Static.WHITE)
    elif draw_toggle:
        pygame.draw.rect(SCREEN, enabled_color,
                         (x + text_rect.width + int(SCREEN_WIDTH / 50), y + int(h / 4), TOGGLE_ADJ, rect_height))
        draw_circle(SCREEN, int(x + text_rect.width + SCREEN_WIDTH / 50), y + int(h / 2), int(h // 4), enabled_color)
        draw_circle(SCREEN, int(x + text_rect.width + SCREEN_WIDTH / 50 + int(h / 4) + TOGGLE_ADJ / 2), y + int(h / 2),
                    int(h // 4), enabled_color)
...
def choose_category(import_status=""):
    ...
    while choose_category_menu:
        ...
        if button('flash-drive.bmp', x + (w / 3) * 2, y, w / 3, h, click, inactive_color=Static.ORANGE,
            ...
            SCREEN.blit(text_surf, text_rect)
            pygame.display.update()
            command_list = ["python3", "check_usb_input.py", config['game_type']]
            usb_input = subprocess.check_output(command_list)
            choose_category_menu = False
...
if __name__ == "__main__":
    ...
    MINI_TEXT = pygame.font.SysFont("Ariel", int(SCREEN_HEIGHT / 40))
